...
public class ListViewAdapterLocal extends ArrayAdapter<File> {
    ...
    public View getView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
        ...
        options.setOnClickListener(e->{
            ...
            popupMenu.setOnMenuItemClickListener(item-> {
                ...
                if(item.getTitle().equals("Properties")){
                    ...
                    properties.putExtra("FILE_SIZE", file.isDirectory() ? Objects.requireNonNull(new java.io.File(file.getLocalPath()).listFiles()).length + "" : file.getSize());
                    ...
                    properties.putExtra("FILE_LPATH", checkIfEmpty(file.getLocalPath()));
                    properties.putExtra("FILE_HASH", checkIfEmpty(file.getHash()));
                    properties.putExtra("FILE_RPATH", checkIfEmpty(file.getRemotePath()));
        ...
        textView2.setText(file.isDirectory() ? Objects.requireNonNull(new java.io.File(file.getLocalPath()).listFiles()).length + "" : getFileSize(file.getSize()) );
